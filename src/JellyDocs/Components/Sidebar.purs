...
renderSidebarSectionItem rootId (Documents title children) = hooks do
  { pageSig } <- useRouter
  ...
  let
    isActiveSig = do
      page <- pageSig
      pure $ case page of
        PageDocument id -> id == rootId <> [ title ]
        _ -> false
  pure do
    el "li" [ "class" := "my-1" ]
      do
        link
          (PageDocument (rootId <> [ title ]))
          [ "class" :=@ do
              isActive <- isActiveSig
              pure $ "p-2 rounded transition-colors block" <> if isActive then " bg-slate-500 text-white" else " hover:bg-slate-200"
          ]
          do
            text $ pure title
